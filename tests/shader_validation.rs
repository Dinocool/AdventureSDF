...
fn sdf_deferred_lit_wgsl_validates() {
    // The deferred lit pass imports the sdf::* graph (bindings + oct for the G-buffer normal
    // decode). Compose the whole graph + validate the default (lit) build AND each `#ifdef`-gated
    // debug-view branch so errors inside them are caught exactly as the GPU pipeline would compile them.
