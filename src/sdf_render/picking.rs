...
/// Result of gizmo picking: which handle of the universal manipulator was hit.
pub struct GizmoPickResult {
    pub distance: f32,
    pub handle: gizmo::HandleId,
    pub hit_pos: Vec3,
}

/// Sphere-trace EVERY handle of the universal manipulator and return the nearest
/// hit (there is no current mode — translate/rotate/scale handles are all live).
/// Handle geometry comes from [`gizmo::Handle`] — the same definition used to
/// draw them — inflated by a pick pad so the thin visuals stay clickable.
pub fn raymarch_gizmo(ray: &Ray, origin: Vec3) -> Option<GizmoPickResult> {
    const PICK_PAD: f32 = 0.04;
    let mut best: Option<GizmoPickResult> = None;

    for handle in gizmo::Handle::all(origin) {
        let mut t = 0.0_f32;
        for _ in 0..128 {
            let p = ray.origin + ray.direction * t;
            let d = handle.sdf(p, PICK_PAD);
            if d < 0.001 {
                if best.is_none() || t < best.as_ref().unwrap().distance {
                    best = Some(GizmoPickResult {
                        distance: t,
                        handle: handle.id,
                        hit_pos: p,
                    });
