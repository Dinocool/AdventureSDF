...
pub fn render_gi_panel(world: &mut World, ui: &mut egui::Ui) {
    ...
    // ReSTIR GI controls: the reservoir knobs (ReSTIR is the live GI path unconditionally).
    {
        ui.separator();
        ui.label(egui::RichText::new("ReSTIR GI").strong());
        if let Some(mut s) = world.get_resource_mut::<crate::voxel::raytrace::RestirSettings>() {
            // ReSTIR is the only GI path (the legacy gather_gi A/B toggle was removed; gather_gi survives as the
            // test-only estimator oracle), so these reservoir knobs are always live.
            ...
            ui.add_enabled_ui(s.di_enabled, |ui| {
