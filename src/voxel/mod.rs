...
/// Cornell box (a few bricks, NO streaming), the canonical GI correctness anchor (colour bleed, an emissive
/// area light, soft shadows) that runs fast. [`VoxelScene::Worldgen`] — the LARGE, streamed, GI-rich
/// procedural terrain (Phase 2.6: sky GI on slopes, multi-bounce fill in deep valleys, emissive lava/crystal
/// colour bleed, the world-cache at scale) — is reachable via the **`V`** toggle. (Worldgen was briefly the
/// default but is heavy and currently being perf-profiled/optimized, so the default is back to Cornell.) The
/// single SSOT knob the streaming + camera-framing systems read to decide which path runs.
#[derive(Resource, Clone, Copy, Debug, PartialEq, Eq, Default)]
pub enum VoxelScene {
    /// Static Cornell box — fully resident, no streaming. The DEFAULT boot scene + the GI correctness anchor.
    #[default]
    Cornell,
    /// Infinite streaming worldgen terrain (the original Stage-3 path). Reachable via the **`V`** toggle; the
    /// large GI-rich stress scene (currently being perf-profiled/optimized — see the worldgen-perf work).
    Worldgen,
}
